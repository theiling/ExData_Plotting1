...
if (!file.exists("household_power_consumption.zip")) {
  ...
  download.file(myfile, destfile="household_power_consumption.zip", method="curl", mode="wb")
} else
...
#data <- read.table("household_power_consumption.txt",sep=";", header=TRUE, skip=66637, nrow=2880)
#data2 <- data[as.character(data$Date) %in% c("1/2/2007", "2/2/2007"),]
